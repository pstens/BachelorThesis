...
In diesem Kapitel werden die Evaluationsergebnisse aus \autoref{chap:eval} auf die Konzeption einer eigenen Android-App angewandt werden.
...
\todo{Um sicherzustellen, dass alle Usability-Heuristiken beachtet werden}
...
\todo{Sequence Diagram zum basic Workflow einfügen}
...
Aufgrund dessen soll diese Funktionalität über zwei dedizierte Icons in der Menüleiste der App angeboten werden.
...
\todo{Bild von Menüleiste aus anderen Apps mit Redo/Undo}
...
\todo{ref auf Material-Guidelines}
...
Dies kann zum Beispiel durch das Ausgrauen der Icons von nicht-ausführbaren Aktionen umgesetzt werden.
\todo{Bild von disabled Icons in Apps}
...
\todo{Irgendein Bild Langklick Geste}
...
Diese ermöglichen es dem Benutzer, bei einem langen Klick auf ein Icon einen kleinen Satz angezeigt zu bekommen, der beschreibt, welche Funktion sich hinter dem Symbol verbirgt.
Ein weiterer Vorteil bei der Benutzung von \emph{Content-Descriptions} liegt in der Unterstützung von Screen-Readern und der Navigation per Keyboard.
...
Dies kann durch die Implementierung eines \emph{Saved States} garantiert werden.
\todo{Saved State android guidelines}
...
Außerdem sollen ausgefüllte Kreise an den Eckpunkten der zur Zeit markierten Formen als \emph{Call for action} dienen, und dem Nutzer so darauf aufmerksam machen, dass die Form mit Hilfe dieser Eckpunkte in ihrer Position und Größe verändert werden kann.
...
\todo{android guidelines zu screen rotation}
