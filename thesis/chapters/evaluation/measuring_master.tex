...
Sollte die App doch in einen Fehlerzustand gelangen, wie bspw. durch den Import eines beschädigten Bildes, wird ein \emph{Toast} angezeigt.
Dieses \emph{Toast} klärt den Nutzer darüber auf, was zu dem Fehler geführt hat, und wie dieser gelöst werden kann.
\todo{Bild vom Toast beim Bildimport} (Nielsen~\autoref{itm:N9}). \\
...
Dies ermöglicht ein schnelles und effizientes Scannen des Bildschirminhaltes, da sofort ersichtlich ist, welche Form zur Zeit ausgewählt ist (Nielsen~\autoref{itm:N12}). \\ 

Das Malen von Formen durch das Klicken und anschließende Ziehen mit einem Finger auf dem Bildschirm ermöglicht eine einfache und effiziente Benutzung der App mit einer Hand (Nielsen~\autoref{itm:N17}).
...
Dies macht das Malen von Formen nicht nur effizienter, sondern beugt auch Fehlern vor, da dieser Bereich sonst vom Finger des Benutzer bedeckt ist. \\
...
\todo{Bilder Portrait und landscape}
...
So führt die fehlerhafte Umsetzung der \emph{Pinch-Geste} dazu, dass der Nutzer beim Zoomen des Bildes unabsichtlich eine Form malt, wenn er zuvor im Zeichen-Modus ist (Nielsen~\autoref{itm:N16}).
Dieser Punkt wirkt sich negativ auf das Anwendungserlebnis aus, da hierdurch nach nahezu jeder Zoom-Aktion eine Undo-Aktion erzwungen wird, um die unabsichtlich-gemalte Form rückgängig zu machen (Nielsen~\autoref{itm:N13}). \\
...
Einzig bei der Benutzung von Gesten zum Zoomen im Bild und der Unterstützung von verschiedenen Bildschirmausrichtungen identifizieren haben sich während der Evaluation Schwachstellen identifizieren lassen.
Besonders die fehlerhafte Gesten-Unterstützung und den damit verbundenen unabsichtlich-gemalten Formen ist ein Kritikpunkt, der das Nutzungserlebnis der App enorm schmälert, da der Benutzer hier dazu ``gezwungen'' wird, nach jeder Zoom-Aktion eine Undo-Aktion zu tätigen.
...
Auch die eingetragenen Messwerte lassen sich im Nachhinein nur mühsam aus dem Bild extrahieren, da diese nicht mehr als Meta-Daten vorliegen, sodass die Weiterverarbeitung in einem nachgeschalteten Dienst wie einer \emph{API} nur erschwert möglich sind.
