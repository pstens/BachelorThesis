...
Die App \mm{} von der Robert Bosch GmbH ist im Google Play-Store unter der Rubrik ``Effizienz'' gelistet.
...
Sobald das gewünschte Projekt ausgewählt wurde, kann der Benutzer durch Ausklappen des Menüs an der linken Seite die Funktionen der App (vgl. \autoref{fig:bmenu}) sehen. \\
...
\begin{figure}[h]
  ...
  \end{subfigure}
  \centering
  \caption{\mm{} bei ausgeklapptem Navigationsmenü und in der Aufmaß-Funktion}
\end{figure}
...
In dieser Oberfläche kann der Benutzer vier verschiedenen Formen (Linie, Viereck, Winkel, Freiform) in das Bild zeichnen und über einen Klick auf die gewünschte Form die zugehörigen Messwerte eintragen.
...
Zusätzlich bietet der erklärende Text im oberen Bereich des Bildschirms (siehe \autoref{fig:bbar}) eine konkrete Hilfestellung zu den möglichen Aktionen, die der Benutzer im aktuellen Systemzustand ausführen kann.
...
Darüber hinaus kann der Nutzer über ein Undo- bzw. Redo-Icon in der Menüleiste (siehe \autoref{fig:bbar}) fehlerhafte Eingaben rückgängig machen oder rückgängig gemachte Aktionen wiederholen (Nielsen~\autoref{itm:N3}).
...
So lassen sich Formen bspw. erst dann löschen, wenn diese zuvor markiert wurde, oder Aktionen rückgängig machen, wenn zuvor eine Benutzeraktion stattgefunden hat (Nielsen~\autoref{itm:N5}). \\
...
Dies macht das Zeichnen von Formen nicht nur schneller, sondern beugt auch Fehlern vor, da dieser Bereich sonst vom Finger des Benutzer verdeckt ist. \\
...
Die Gesten-Unterstützung zur Navigation im Bild ist jedoch nicht so gut umgesetzt worden.
So führt die fehlerhafte Umsetzung der \emph{Pinch-Geste}\urlnote{http://www.giga.de/unternehmen/apple/news/pinch-to-zoom-microsoft-und-samsung-wollen-vorsorge-patent/}{22.11.2017} dazu, dass der Nutzer beim Zoomen des Bildes unabsichtlich eine Form zeichnet, wenn er zuvor im Zeichen-Modus ist (Nielsen~\autoref{itm:N16}).
...
Das Exportieren des bearbeiteten Bildes führt dazu, dass alle zuvor eingetragenen Messwerte nicht mehr trivial auslesbar sind, da diese nicht als Meta-Daten vorliegen (\autoref{itm:export}).
...
Zusammenfassend lässt sich festhalten, dass die App \mm{} von der Robert Bosch GmbH die Usability-Heuristiken nach Nielsen größtenteils erfüllt.
